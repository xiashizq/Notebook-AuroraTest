...
void ZFileWidget::setTempFile()
{
    ...
    QString srcPath = mPathDiffModel.srcFileInfo().absoluteFilePath();
    QString dstPath = mPathDiffModel.dstFileInfo().absoluteFilePath();
QString srcTempFilePath = QString("%1%2%3").arg(tempDir).arg(QDir::separator()).arg(ZFile::randString());
QString dstTempFilePath = QString("%1%2%3").arg(tempDir).arg(QDir::separator()).arg(ZFile::randString());
